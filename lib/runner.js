...
Runner.prototype.setupGlobals_ = function(driver) {
  ...
  global.newBrowser = function() {
    var driver = this.driverprovider_.getNewDriver();

    driver.manage().timeouts().setScriptTimeout(this.config_.allScriptsTimeout);
    ...
    global.browsers.push(browser);

    return browser;
  }.bind(this);

  global.quitBrowser = function(browser) {
    var index = global.browsers.indexOf(browser);

    if(index === -1)
      throw new Error('browser does not exist');

    global.browsers.splice(index, 1);
    this.driverprovider_.quitDriver(browser.driver);
  }.bind(this);
